...
export class UserService {
  ...
  async register(request: RegisterUserRequest): Promise<UserResponse> {
    this.logger.debug(`register user request: ${JSON.stringify(request)}`);
  ...
  async login(request: LoginUserRequest): Promise<UserResponse> {
    ...
    if (!user) {
      throw new HttpException('Username or password is invalid user', 401);
    }
    ...
    if (!isPasswordValid) {
      throw new HttpException('Username or password is invalid password', 401);
    }
